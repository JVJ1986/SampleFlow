...
export default function Navbar() {
  const { session, page, setPage, handleSignOut } = useContext(AppContext);
  ...
  return (
    <nav style={s.nav}>
      ...
      <div style={s.search}>
        ...
        <input style={{ border:'none', background:'none', outline:'none', fontSize:13, color:'#111', width:140 }} placeholder="Search..." id="globalSearchInput" />
      </div>
      <div style={{ display:'flex', alignItems:'center', gap:12, marginLeft:14 }}>
        ...
        <div style={{ textAlign:'right', lineHeight:1.4 }}>
          ...
          <span style={{ fontSize:11, color:'#999', cursor:'pointer' }} onClick={handleSignOut}>[→ SIGN OUT]</span>
        </div>
